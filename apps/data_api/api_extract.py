...
def raw_2_modelformat(all_data):
    ...
    jslist = []
    for i in range(len(all_data1)):
        dic = {"model":'data_api.Drug', 'fields':all_data2[i]}
        jslist.append(dic)
    
    with open('./drug.json', 'w', encoding='UTF-8-sig') as f:
        json.dump(jslist, f, indent=4,ensure_ascii=False)
    

def main():
    apikey = key.API_CONFIG['key']
    url = key.API_CONFIG['url']
